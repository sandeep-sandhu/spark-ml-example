...
object ModelData extends {
  ...
  def saveDataFrameToCSV(
    testResultDF: org.apache.spark.sql.DataFrame,
    outputFileName: String
  ): Unit = {

    // first, remove any vector columns since they cannot be written to a csv file:
    var droppedTestDF = testResultDF.drop(
      "features",
      "numericalfeatures",
      "scaledfeatures",
      "rawPrediction",
  ...
  def readFromJdbcConn(
    ...
    logger.info(
      "Loaded data from database table \'%s\' into a dataframe with %d partitions"
        .format(tableName, dbdata_df.rdd.getNumPartitions)
    );
